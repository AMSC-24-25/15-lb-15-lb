...
# 假设网格高度 Ny 已知（比如从实验数据中推断）
Ny = 100  # 如果不确定，可以动态传入或修改代码

# 循环读取并绘制速度场
for iteration in range(start_iter, end_iter + 1, step):
    # 动态生成文件名，假设每次迭代的结果文件以迭代次数命名
    filename = f"lbm_results_{iteration}.csv"
    ...
    try:
        ...
        if Nx * Ny != total_points:
            raise ValueError(f"Data size ({total_points}) does not fit grid dimensions Nx={Nx}, Ny={Ny}.")
